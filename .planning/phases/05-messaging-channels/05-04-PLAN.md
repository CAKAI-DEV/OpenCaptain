...
must_haves:
  ...
  prerequisites:
    - file: "src/features/tasks/tasks.service.ts"
      function: "getTask"
      note: "Required for visibility check on task comments - exists from Phase 4"
    - file: "src/features/deliverables/deliverables.service.ts"
      function: "getDeliverable"
      note: "Required for visibility check on deliverable comments - exists from Phase 4"
---
...
<task type="auto">
  ...
  <action>
**Prerequisites verification:** Before implementing, verify that the following functions exist from Phase 4:
- `getTask` from `src/features/tasks/tasks.service.ts` (returns task with projectId for visibility check)
- `getDeliverable` from `src/features/deliverables/deliverables.service.ts` (returns deliverable with projectId for visibility check)

If these functions do not exist, this plan cannot proceed - report a blocker.
